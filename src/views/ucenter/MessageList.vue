  ...
  <div class="container-body message-center" :style="{ width: proxy.globalInfo.bodyWidth + 'px' }">
    ...
    <div class="message-panel">
        ...
        <div class="tab-list-info">
          <el-tabs :model-value="activeTabName">
            <el-tab-pane name="reply">
              <template #label>
                <div class="itemone tab-item">
                  <span @click="changeTab('reply')">回复我的</span>
            ...
            <el-tab-pane name="likePost">
              <template #label>
                <div class="tab-item">
                  <span @click="changeTab('likePost')">赞了我的文章</span>
            ...
            <el-tab-pane name="downloadAttachment">
              <template #label>
                <div class="tab-item">
                  <span @click="changeTab('downloadAttachment')">下载了我的附件</span>
            ...
            <el-tab-pane name="likeComment">
              <template #label>
                <div class="tab-item">
                  <span @click="changeTab('likeComment')">赞了我的评论</span>
            ...
            <el-tab-pane name="sys">
              <template #label>
                <div class="tab-item">
                  <span @click="changeTab('sys')">系统消息</span>
      ...
      <div class="message-list">
        <DataList :dataSource="messageInfo" @loadData="loadMessage" noDataMsg="空空如也" :loading="loading">
          <template #default="{ data }">
            ...
            <div class="message-item" v-if="data.messageType == 1">
              ...
              <div class="message-content">
                <div>
                  ...
                  对我的文章【<router-link class="a-link" :to="`/post/${data.articleId}`">{{ data.articleTitle
                  }}</router-link>】发表了评论
                  <span class="create-time">{{ data.createTime }}</span>
                </div>
            ...
            <div class="message-item" v-if="data.messageType == 3">
              ...
              <div class="message-content">
                <div>
                  <router-link class="a-link" :to="`/user/${data.sendUserId}`">@{{ data.sendNickName }}</router-link>
                  在文章【<router-link class="a-link" :to="`/post/${data.articleId}`">{{ data.articleTitle
                  }}</router-link>】中赞了我的评论
                  <span class="create-time">{{ data.createTime }}</span>
                </div>
                <div class="reply-content" v-html="data.messageContent"></div>
            ...
            <div class="message-item" v-if="data.messageType == 2">
              ...
              <div class="message-content">
                <div>
                  <router-link class="a-link" :to="`/user/${data.sendUserId}`">@{{ data.sendNickName }}</router-link>
                  赞了我的文章【<router-link class="a-link" :to="`/post/${data.articleId}`">{{ data.articleTitle
                  }}</router-link>】
                  <span class="create-time">{{ data.createTime }}</span>
                </div>
              </div>
